import React from "react";
import tic from "../assets/tic.png";
import weather from "../assets/weather.png";
import food from "../assets/food.png";
import protfolio from "../assets/protfolio.png";
import chat from "../assets/chat.png";
const projects = [
  {
    id: 1,
    name: "Tic-Tac-Toc",
    technologies: "HTML,CSS & JavaScript",
    image: tic,
    github: "https://github.com/Sonmatreya/Tic-Tac-Toe",
  },
  {
    id: 2,
    name: "Weather-App",
    technologies: "HTML,CSS & Javascript",
    image: weather,
    github: "https://github.com/Sonmatreya/Weather-App",
  },
  {
    id: 3,
    name: "Chat-App",
    technologies: "MERN Stack",
    image: chat,
    github: "https://github.com/Sonmatreya/Chat-App",
  },
  {
  id: 4,
  name: "Food-App",
  technologies: "MERN Stack",
  image: food,
  github: "https://github.com/Sonmatreya/Food-App",
  },
  {
  id: 5,
  name: "Protfolio ",
  technologies: "MERN Stack",
  image: protfolio,
  // github: "https://github.com/Sonmatreya/Chat-App",
  },
];

const Projects = () => {
  return (
    <div className="bg-black text-white py-20" id="project">
      <div className="container mx-auto px-8 md:px-16 lg:px-24">
        <h2 className="text-4xl font-bold text-center mb-12">My Projects</h2>
        <div className="flex flex-wrap justify-center gap-8">
          {projects.map((project, index) => (
            <div
              key={project.id}
              className="bg-gray-800 p-6 rounded-lg hover:shadow-lg transform transition-transform duration-300 hover:scale-105 w-96"
            >
              <img
                src={project.image}
                alt={project.name}
                className="rounded-lg mb-4 w-full h-48 object-cover"
              />
              <h3 className="text-2xl font-bold mb-2">{project.name}</h3>
              <p className="text-gray-400 mb-4">{project.technologies}</p>
              <a
                href={project.github}
                className="inline-block bg-gradient-to-r from-red-800 to-yellow-500 text-white px-4 py-2 rounded-full"
                target="_blank"
                rel="noopener noreferrer"
              >
                GitHub
              </a>
            </div>
          ))}
        </div>
      </div>
    </div>
  );
};

export default Projects;
